  ...
  return (
    <div className="space-y-4">
      ...
      <div className="border-b border-gray-200 dark:border-gray-700 pb-0">
        {/* Mobile: Dropdown selector + Settings */}
        <div className="md:hidden flex items-center gap-2 pb-3">
          <select
            value={view}
            onChange={(e) => {
              const newView = e.target.value as typeof view;
              if (newView === 'sprint' && !activeSprint) return;
              setView(newView);
            }}
            className="flex-1 px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-slate-800 text-gray-900 dark:text-white font-medium text-sm"
          >
            <option value="backlog">Product Backlog</option>
            <option value="sprint" disabled={!activeSprint}>Active Sprint {activeSprint ? '• Live' : '(none)'}</option>
            <option value="planning">Plan Future Sprint</option>
            <option value="risks">Risks</option>
            <option value="completed">Completed Sprints</option>
            <option value="retros">Retrospectives</option>
          </select>
          <button
            onClick={() => setShowProjectDetailsModal(true)}
            className="p-2.5 rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300"
